...
Runs on every push to `main` and every pull request targeting `main`. Steps: checkout, Node setup (version pinned via `.nvmrc`, not hardcoded in the workflow — see that file), install, Playwright Chromium install, lint, typecheck, test (the qa suite), build, verify:esm.

**Added 2026-07-14 (REV-022 remediation)**: a `npx playwright install --with-deps chromium` step now runs immediately after `Install dependencies` and before `Lint`/`Test`. INC-12 added a permanent, committed Playwright-based browser regression test (`tests/browser/iconSizing.playwright.test.ts`, per `docs/handoff.md`'s INC-12 bug-fix-cycle notes) that verifies real-browser CSS custom-property resolution for icon sizing — something jsdom (this repo's default Vitest environment) cannot do at all, per the test file's own top-of-file comment. Because `vite.config.ts`'s test include glob (`tests/**/*.{test,spec}.{ts,tsx}`) matches this file with no special-casing, it already runs as part of the ordinary `npm test` step CI invokes on every push/PR — it is not a separately opted-in suite, and reviewer independently confirmed (`docs/review-log.md` REV-022) that a missing Chromium binary fails this step loudly (`browserType.launch: Executable doesn't exist...` surfacing as a failed `beforeAll` hook, non-zero `vitest` exit code) rather than silently skipping. `ubuntu-latest` GitHub Actions runners do not ship a pre-installed Chromium at a path Playwright's default resolution would find, so without this step the very next push/PR to `main` after INC-12 merges would make the "Test (qa suite)" step genuinely fail. The install step uses Playwright's own documented CI installation command, `npx playwright install --with-deps chromium` (`--with-deps` additionally installs the OS-level shared libraries Chromium needs on a bare `ubuntu-latest` runner, e.g. via `apt-get`, which a from-scratch GitHub-hosted runner does not have preinstalled) — the browser version installed always matches whatever `playwright` version `npm ci` just resolved from the committed `package-lock.json` (currently `^1.56.1`, a devDependency only, not shipped to production/the browser bundle), so this workflow file does not itself pin or hardcode a Playwright/Chromium version. No new environment variables or GitHub Actions secrets are required — the install downloads a public browser binary over the network, same trust boundary as any other `npm ci`-installed devDependency, and needs no `MAP_API_KEY`/`PLAYWRIGHT_BROWSERS_PATH`-style configuration (the default per-runner cache path Playwright resolves to is sufficient since install and test run in the same job/runner instance). It is gated behind the same `pkgcheck` condition as the other real steps (no-op, not a failure, on the pre-INC-1 skip branch, which remains dead code in practice per the Phase 4 note below).

**Verification performed for this change**: this session does not have network/GitHub Actions execution access to run a real `ubuntu-latest` install-from-scratch dry run of `npx playwright install --with-deps chromium` against this repo's actual CI runner. Verification performed instead: (1) confirmed `.github/workflows/ci.yml`'s YAML parses correctly (`python3 -c "import yaml; yaml.safe_load(open('.github/workflows/ci.yml'))"`, no syntax errors); (2) confirmed the install command matches Playwright's own documented CI installation pattern exactly (`npx playwright install --with-deps <browser>`), with `chromium` naming only the one browser this test file actually launches (`tests/browser/iconSizing.playwright.test.ts` imports only `chromium` from `playwright`, confirmed by direct read — no `firefox`/`webkit` install is needed); (3) ran the full `npm test` suite locally in an environment that happens to already have a matching Chromium binary available (this sandbox has one pre-installed at `/opt/pw-browsers`, with `PLAYWRIGHT_BROWSERS_PATH` already set in the shell environment, matching the pinned `playwright@1.56.1` devDependency version) — all 40 test files / 598 tests passed, including `tests/browser/iconSizing.playwright.test.ts`'s 7 tests, confirming the test itself is genuinely satisfied once a matching Chromium binary is reachable, which is exactly the condition the new CI step is intended to establish on a GitHub-hosted runner. **What remains unconfirmed until a real GitHub Actions run happens**: that `npx playwright install --with-deps chromium` itself succeeds against a genuinely fresh, from-scratch `ubuntu-latest` runner (network reachability to Playwright's CDN from GitHub's infrastructure, `apt-get`-based OS dependency installation succeeding under whatever base-image package set that runner image currently ships, and install step runtime/cache behavior across repeated runs). Reviewer/QA should confirm on the next real push/PR to `main` that the "Install Playwright Chromium" step and the subsequent "Test (qa suite)" step both go green — this is the honest, outstanding verification gap this change carries into a real CI run.

**Added 2026-07-12 (REV-018 recurrence prevention)**: a `verify:esm` step now runs after `build`, invoking `npm run verify:esm` (`scripts/verify-esm-resolution.mjs`, added by dev per `docs/review-log.md`'s "Post-Closure Incident Audit — REV-018" recommendation). This step compiles the backend-reachable code tree (`api/**`, non-frontend `src/**`) with `tsc` targeting `module: ESNext` and then imports each compiled `api/*.ts` handler with a real, unbundled Node `import()` — genuine Node ESM module resolution, not the bundler-style resolution `tsc --noEmit`/`vite build`/`vitest` use elsewhere in this pipeline. This is the exact check that would have caught the production incident recorded in section 10's Release Log (extensionless relative imports that resolved fine under bundler tooling but crashed every `api/*.ts` handler on Vercel with `ERR_MODULE_NOT_FOUND`), and it is now a required gate: a non-zero exit fails the CI job overall, the same as a failing test. It requires no environment variables, no network access, and no Vercel credentials or `VERCEL_TOKEN` — deliberately, per reviewer's REV-018 recommendation to get this correctness guarantee without adding Vercel credentials to CI for a full `vercel build` step (see "What CI does NOT do" below). It is gated behind the same `pkgcheck` condition as the other real steps, so it is a no-op (not a failure) on the pre-INC-1 skip branch, which is now dead code in practice (see the Phase 4 closure note below) but kept for the same reason as the other steps.
...
- (Added post-closure, REV-018) A `verify:esm` npm script (`node scripts/verify-esm-resolution.mjs`) that CI runs unconditionally after `build`.
- (Added INC-12/REV-022) Any committed `tests/browser/*.playwright.test.ts` file needs only the `playwright` devDependency and a real Chromium binary — CI supplies the latter itself via the `npx playwright install --with-deps chromium` step, so dev/QA do not need to provision a browser binary themselves in the repo or in CI configuration.
...
**Re-confirmed 2026-07-12 (REV-018 CI wiring)**: after adding the `verify:esm` step above, re-ran the full simulated CI sequence in the same order the workflow runs it — `npm ci` -> `npm run lint` -> `npm run typecheck` -> `npm test` -> `npm run build` -> `npm run verify:esm` — all six steps exited 0 against the current codebase (36 test files, 433 tests still passing; `verify:esm` reports all 4 `api/*.ts` handlers `OK` and prints its `PASS` summary). No new GitHub Actions secrets were added or are required — `verify:esm` only needs the locally-installed `tsc` binary and Node's built-in module loader, no network access, no Vercel credentials, consistent with reviewer's REV-018 recommendation to avoid a full `vercel build` step in CI (see "What CI does NOT do" below).

**Re-confirmed 2026-07-14 (REV-022 Playwright CI wiring)**: added the `npx playwright install --with-deps chromium` step described above, positioned right after `Install dependencies` and before `Lint`/`Test` (the only later step that needs it is `Test`, since `tests/browser/iconSizing.playwright.test.ts` runs inside `npm test`). Could not execute a genuine fresh `ubuntu-latest`-runner install dry run from this session (no GitHub Actions execution access here) — see section 5's "Verification performed for this change" note above for exactly what was and was not verified, including a full local `npm test` run (40 test files, 598 tests, all passing, including the 7 tests in `tests/browser/iconSizing.playwright.test.ts`) against a Chromium binary already available in this sandbox. This closes the immediate gap reviewer flagged in `docs/review-log.md` REV-022 (major): merging INC-12 to `main` without this step would have made CI's "Test (qa suite)" step genuinely fail on the next push/PR. No new GitHub Actions secrets were added or are required for this step. **Reviewer should confirm REV-022 closed once a real push/PR to `main` shows the "Install Playwright Chromium" and "Test (qa suite)" steps both passing** — that confirmation cannot happen from within this session.
...
| 2026-07-12 | REV-018 recurrence-prevention CI wiring: added a required `verify:esm` step to `.github/workflows/ci.yml`, running `npm run verify:esm` (`scripts/verify-esm-resolution.mjs`, built and verified by dev) after the existing `build` step, gated behind the same `pkgcheck` condition as the other real gates. A non-zero exit fails the CI job the same way a failing test does. Updated section 5's step list and added a dedicated explanatory paragraph describing what the step does, why it's the exact check that would have caught the REV-018 incident, and why it needs no new secrets (no network access, no Vercel credentials — consistent with reviewer's REV-018 recommendation to avoid a full `vercel build` step in CI). Also updated the workflow file's own top-of-file scope comment. Re-ran the full simulated CI sequence (`npm ci` -> `lint` -> `typecheck` -> `test` -> `build` -> `verify:esm`) locally, all six steps exiting 0 (433 tests passing, all 4 API handlers `OK` under `verify:esm`). No new GitHub Actions secrets added or required. | Reviewer's REV-018 recommendation (`docs/review-log.md`, "Post-Closure Incident Audit — REV-018") called for wiring the already-built `verify:esm` script into CI as a required gate so this exact regression class can never reach production again without CI catching it first |
| 2026-07-14 | REV-022 remediation: added a `npx playwright install --with-deps chromium` step to `.github/workflows/ci.yml`, positioned after `Install dependencies` and before `Lint`/`Test` (the only step that needs it, since `tests/browser/iconSizing.playwright.test.ts` runs as part of the ordinary `npm test` step per `vite.config.ts`'s test-include glob), gated behind the same `pkgcheck` condition as the other real steps. Uses Playwright's own documented CI install command; installs only `chromium` (the sole browser the test file imports); browser version tracks whatever `playwright` version `npm ci` resolves from the committed `package-lock.json` (currently `^1.56.1`), not hardcoded separately. Updated section 5's step list, top-of-file workflow comment, and "Contract this runbook assumes dev will deliver" list to document this. No new GitHub Actions secrets or environment variables added or required. Verified locally: YAML parses correctly, install command matches Playwright's documented pattern, and a full local `npm test` run (40 test files, 598 tests) passed including the 7 tests in `tests/browser/iconSizing.playwright.test.ts`, using a Chromium binary already available in this sandbox — could not execute a genuine from-scratch `ubuntu-latest`-runner install dry run from this session (no GitHub Actions execution access here); this is flagged as an open verification item for the next real push/PR to `main` (see section 5). No secret values added anywhere; no application code touched. | Reviewer's `docs/review-log.md` REV-022 (major): INC-12 added a permanent, committed Playwright browser test with no corresponding CI Chromium install step, which would make CI's existing "Test (qa suite)" gate genuinely fail (not silently skip) on the next push/PR to `main` once INC-12 merges |
