...
Runs on every push to `main` and every pull request targeting `main`. Steps: checkout, Node setup (version pinned via `.nvmrc`, not hardcoded in the workflow — see that file), install, lint, typecheck, test (the qa suite), build, verify:esm.
