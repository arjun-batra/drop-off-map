...
None of the above blocks writing this runbook or the CI workflow file, but items 1-3 must be resolved before INC-1's code can actually be deployed (the CI quality gate can run without them; the Vercel deploy step cannot).
...
### 9.4 Rollback tested
**No** — no deployment exists yet to test against (pre-INC-1, as of this writing). This procedure must be dry-run tested against the first real production deployment (recommend doing so right after INC-1 merges, as a deliberate low-stakes drill, before real user traffic depends on it) and the result recorded in section 10.
