...
- `POST /api/drop-off-search`:
  ...
  interface DropOffSearchRequest {
    ...
    maxDetourMinutes: number; // FR-002, user-entered, no server default. HARD-REQUIRED (category 2,
                               // below) -- absence itself is `invalid_input`; there is no sensible
                               // default detour threshold to fall back to.
    avoidTolls: boolean;      // (2026-07-12, FR-018) per-session checkbox, no server default/config
                               // *value* (never sourced from AppConfig). OPTIONAL-WITH-DEFAULT
                               // (category 1, below) -- a missing field defaults to `false`; only a
                               // present-but-non-boolean value is `invalid_input`.
  }
...
**Per-request field categories (REV-023 clarification, added 2026-07-15)**: "no server default/config" on a `DropOffSearchRequest` field means only that the value is sourced from the request, never from `AppConfig` — it says nothing on its own about whether the field may be *omitted*. Two distinct categories exist among this request's fields, and they behave oppositely on omission:
  1. **Optional-with-default** — the field has a specified default; a missing/absent field is treated as that default, and only a *present-but-malformed* value (wrong type/shape) is rejected as `invalid_input`. `avoidTolls` is this category (default `false`).
  2. **Hard-required** — the field has no sensible default; its *absence* is itself `invalid_input`, not just a malformed present value. `maxDetourMinutes` is this category (no default exists for a detour-minutes ceiling).

  `start`, `driverDestination`, and `passengerDestination` are also hard-required (category 2) — there is no meaningful default location, so a missing/malformed value on any of the three is `invalid_input`.

  **Default for any new per-request field added going forward (e.g., INC-14's toll-reentry-answer field), unless this document states otherwise for that specific field**: category 2 (hard-required) is the default assumption for fields with no natural fallback value (locations, thresholds); category 1 (optional-with-default) applies only where a specific, stated default value makes the field meaningful when omitted (e.g., a checkbox-style boolean whose unchecked state is itself a valid, common case). Whichever category a new field falls into, its interface comment must name the category explicitly (as `avoidTolls`/`maxDetourMinutes` now do above) rather than reusing the ambiguous bare phrase "no server default" on its own.
...
| Date | Change | Reason |
|---|---|---|
| 2026-07-15 | **Documentation-only clarification, section 5.2, no behavior change.** The identical comment phrase "no server default/config" was used for both `avoidTolls` and `maxDetourMinutes` in `DropOffSearchRequest`, but the two fields behave oppositely on omission (`avoidTolls` defaults to `false` when absent, only a present-but-non-boolean value is `invalid_input`; `maxDetourMinutes`'s absence itself is `invalid_input`, per `parseMaxDetourMinutes`'s shipped behavior) — the phrase alone did not distinguish these. Added an explicit two-category framework directly under the `DropOffSearchRequest` interface: **category 1, optional-with-default** (missing field defaults to a stated value; only a malformed present value is `invalid_input` — `avoidTolls` is this category) and **category 2, hard-required** (absence itself is `invalid_input` — `maxDetourMinutes`, `start`, `driverDestination`, `passengerDestination` are this category). Updated the `avoidTolls`/`maxDetourMinutes` interface comments to name their category explicitly instead of reusing the ambiguous bare phrase. Recorded the default going forward: any new per-request field (e.g., INC-14's toll-reentry-answer field) is category 2 (hard-required) unless this document states a specific default value for it, in which case it is category 1. **FR/NFR coverage**: unchanged — no FR/NFR gained or lost coverage; this is a documentation-clarity fix only, no code/behavior change. | Reviewer (`docs/review-log.md` REV-023, filed during the INC-13 audit) found the shared phrasing produces opposite real behavior for two fields in the same document and flagged this as a `[DESIGN-GAP]` that would recur for INC-14's new per-request field(s) if left unaddressed; tech-lead closes REV-023 with this one-line-scoped amendment |
