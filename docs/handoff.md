...
# Handoff: INC-10 — Google Maps JavaScript API rendering (FR-022)

Status: implemented, smoke-tested (via mocks — no real `GOOGLE_MAPS_JS_API_KEY` available in this sandbox, see "Known limitations" below), ready for QA. Covers FR-022. Gate 3 for the INC-10-14 extension was confirmed passed (2026-07-13) before this increment started — see `docs/design.md`'s top-of-file status line and section 1.4.

New config key: `GOOGLE_MAPS_JS_API_KEY` (optional, `string | null`, browser-exposed via `GET /api/config/public` — the first and only intentionally client-exposed Google credential in this app, per DEC-7/design.md section 7.2). Retired config keys: `MAP_TILE_URL_TEMPLATE`, `MAP_TILE_ATTRIBUTION` (INC-9/Leaflet-specific, per design.md section 7.2's "Cleanup" instruction).

## What was built

### Config

- **`src/config/schema.ts`**: removed `mapTileUrlTemplate`/`mapTileAttribution` from `AppConfig`/`PublicConfig`; added `googleMapsJsApiKey: string | null` to both, with a doc comment recording DEC-7's rationale (distinct threat model from `mapApiKey`, intentionally included in `PublicConfig`, optional/nullable so an unconfigured deployment degrades gracefully rather than hard-failing config load).
- **`src/config/loader.ts`**: removed the two `MAP_TILE_*` `parseOptionalString` calls; added `GOOGLE_MAPS_JS_API_KEY` via the same `parseOptionalString` helper (no `problems.push` on either branch — unset is a valid "map view disabled" configuration, same pattern INC-9 established).
- **`src/config/publicConfig.ts`**: `toPublicConfig()` now returns `googleMapsJsApiKey` instead of the two removed fields.
- **`.env.example`**: replaced the `MAP_TILE_URL_TEMPLATE`/`MAP_TILE_ATTRIBUTION` section with `GOOGLE_MAPS_JS_API_KEY`, documenting that it must be a **distinct** key from `MAP_API_KEY` and the Google Cloud Console restriction each needs (HTTP referrer + Maps-JS-API-only for the new key; the existing guidance for `MAP_API_KEY` is unaffected).

### Frontend

**`src/frontend/components/MapView.tsx`** (rewritten) — same functional contract as the INC-9 Leaflet version (design.md section 10's explicit instruction: "rendering-library swap only"), now built on Google's Maps JavaScript API:

- Loaded via **`@googlemaps/js-api-loader`'s newer functional API** (`setOptions`/`importLibrary`), not the deprecated `Loader` class and not a hand-rolled `<script>` tag — see "Library/approach chosen" below for why.
- `disableDefaultUI: true` + `zoomControl: true` only — Street View, map-type switcher, and fullscreen control stay hidden, per ux-spec.md section 6.7's explicit reasoning (small supporting panel, not a standalone map product).
- A custom muted map style (`src/frontend/components/mapStyle.ts`, new file) — POI and transit-layer visibility off, desaturated road/water/label colors — passed via `MapOptions.styles`, functionally replacing INC-9's CARTO "Positron" basemap intent with zero added provider cost (same API load, just a styling parameter).
- **Custom rank-colored markers, not Google's default red pin**: built at runtime as small SVG data-URI icons (`buildMarkerIcon`), matching the exact same rank-1/rank-other/fallback color coding the candidate cards already use — rank-1 gets `--color-brand-primary`, ranks 2-3 get `--color-text-secondary`, the fallback state's single candidate gets `--color-warning-border`, and a highlighted marker gets a `--color-focus-ring`-colored halo ring. Colors are read from `tokens.css` at runtime via `getComputedStyle` rather than hardcoded hex literals, since an SVG `<img>`-style icon can't take a CSS class the way the old Leaflet `divIcon` could — this is the equivalent mechanism for the project's "reference tokens, not inline literals" rule, not a departure from it.
- Route polyline (`google.maps.Polyline`) and one `google.maps.Marker` per candidate, `map.fitBounds()` over both, tap-to-highlight (`marker.addListener("click", ...)` → `onSelectCandidate(rank)`), and a highlight-only effect that calls `marker.setIcon(...)` on `highlightedRank` changes without recreating/remounting any marker — same "no remount on highlight" behavior as the Leaflet version.
- **Handles the async-load failure path explicitly**: unlike Leaflet (bundled, synchronous), the Maps JavaScript API loads over the network, so a load failure surfaces as a rejected Promise, not a thrown render/effect error — an `ErrorBoundary` alone cannot catch that. `MapView` therefore wraps its own load logic in a try/catch and renders `null` on failure (in addition to `ResultsScreen`'s existing `ErrorBoundary fallback={null}` wrapper, which still covers any *synchronous* render error as a second, independent layer) — preserves ux-spec.md section 6.7's "fail silently, simply omit the panel" requirement for this new async path.

**`src/frontend/components/mapStyle.ts`** (new) — `MUTED_MAP_STYLE: google.maps.MapTypeStyle[]`, the Styled-Maps JSON array (POI/transit hidden, desaturated palette).

**`src/frontend/components/MapView.css`** (rewritten) — Leaflet-specific rules (attribution-control font-size, `.map-view__marker*` classes) removed entirely, since markers are now plain image icons with no DOM/CSS surface for this stylesheet to target. Only the panel container remains, now using `--radius-lg`/`--shadow-lg` per ux-spec.md section 6.7's 2026-07-12 update ("the map is one of this screen's two elevated/primary surfaces").

**`src/frontend/styles/tokens.css`** — added `--radius-lg: 20px` and `--shadow-lg: 0 4px 16px rgba(0, 0, 0, 0.16)` (ux-spec.md section 2.4, previously undefined in this codebase). Scoped narrowly: only `MapView.css` consumes these so far. Rolling `radius-lg`/`shadow-lg` out to the top-pick candidate card (ux-spec.md section 6.4, also flagged as due for this treatment) is part of the broader, FR-less "item 5" UI-modernization pass, not FR-022/INC-10 scope — left untouched deliberately, flagged here rather than silently expanded into.

**`src/frontend/components/ResultsScreen.tsx`** — `mapConfig` prop narrowed from `Pick<PublicConfig, "mapTileUrlTemplate" | "mapTileAttribution">` to `Pick<PublicConfig, "googleMapsJsApiKey">`; `showMap` now checks `!!mapConfig.googleMapsJsApiKey` (single key, in place of the old two-key AND check); `<MapView>` now receives `apiKey={mapConfig.googleMapsJsApiKey}` instead of `tileUrlTemplate`/`tileAttribution`. No other change — the `ErrorBoundary fallback={null}` wrapper, `showMap`'s status-gating logic (`ranked`/`fallback` + `response.route` present), and the tap-to-highlight scroll/flash wiring are all byte-for-byte unchanged.

### Dependencies

- **`package.json`**: added `@googlemaps/js-api-loader` (dependency) and `@types/google.maps` (devDependency, ambient global types). **Removed** `leaflet` (dependency) and `@types/leaflet` (devDependency) via `npm uninstall`, so `package-lock.json` is correctly regenerated rather than hand-edited.
- **`tsconfig.json`**: added `"google.maps"` to the restrictive `types` array (the `@types/google.maps` package is a pure ambient global-namespace declaration, not a module — it has to be explicitly opted into `types`, same reason `"node"` is already there).
- **`eslint.config.js`**: added `google: "readonly"` to the `.ts`/`.tsx` globals block, since `google` (used only in type positions, e.g. `typeof google.maps.Size`) isn't a recognized ESLint global by default and was flagged by the base `no-undef` rule.

## Library/approach chosen for loading Google Maps JS

**`@googlemaps/js-api-loader` (v2.1.1), using its newer functional API (`setOptions`/`importLibrary`), not the older `Loader` class (deprecated in this same package) and not a hand-rolled `<script>` tag.**

Reasoning:
- This is a real, actively-maintained, Google-published npm package specifically for this problem — not an unnecessary dependency for something trivial. Hand-rolling script-tag injection correctly (single-load guarantee across repeated component mounts, avoiding a duplicate `<script>` tag if `MapView` unmounts/remounts across searches, race-safe callback wiring) is exactly the kind of thing a small, well-maintained library does correctly that a bespoke implementation risks getting wrong — consistent with the task's own guidance and this project's existing precedent (e.g. INC-1 didn't hand-roll HMAC/cookie parsing).
- The functional API (`setOptions({ key, v: "weekly" })` once, then `importLibrary("maps" | "marker" | "core")` per needed library) is Google's own currently-recommended pattern (the `Loader` class is explicitly marked `@deprecated` in this same package's own type definitions) and maps cleanly onto this app's need for `Map`/`Polyline` (`"maps"`), `Marker` (`"marker"`), and `LatLngBounds`/`Size`/`Point` (`"core"`).
- `importLibrary()` internally caches per-library-name promises and de-dupes the actual script injection — calling `setOptions`/`importLibrary` again from a second `MapView` mount (e.g. a second search in the same session) does not re-inject the script or double-load the API; this is the exact "script-tag race condition" and "single-load guarantee" concern flagged in the task, handled by the library rather than by this component.
- Deliberately **not** `AdvancedMarkerElement`: Google's newer marker element requires a Map ID (`mapId`) configured in Google Cloud Console to render at all, which would be an undocumented config requirement not present anywhere in `docs/design.md` section 7's schema or `docs/requirements.md`'s Configuration table (only `GOOGLE_MAPS_JS_API_KEY` is specified). ux-spec.md section 6.7 itself names `AdvancedMarkerElement` only as one example ("...or an equivalent custom Icon/Symbol"), so using the classic `google.maps.Marker` with a custom SVG `Icon` is a literal, explicitly-sanctioned reading of the spec that introduces zero new config keys.

## How to smoke-test

1. `npm install`, fill in `.env.local` (a real `GOOGLE_MAPS_JS_API_KEY` is needed to see the actual map render visually — see "Known limitations" below for what could/couldn't be verified in this sandbox without one).
2. Leave `GOOGLE_MAPS_JS_API_KEY` unset to confirm the "map view disabled" path: `GET /api/config/public` returns `googleMapsJsApiKey: null`, and a `ranked`/`fallback` Results-screen response renders with the map panel entirely absent (candidate cards unaffected).
3. Set a real key, run a search that returns `status: "ranked"` or `"fallback"` — the map panel should render between the trip summary and the candidate cards, with custom colored pins (not red default pins), only a zoom control visible (no Street View/map-type/fullscreen buttons), a muted/POI-light basemap, and tapping a pin should scroll/flash the matching card below.

## Smoke test performed

- `npm run typecheck`, `npm run lint`, `npm run build` — all clean on `src/`/`api/`/`scripts/`. (`npm run typecheck`'s only remaining errors are in `tests/`, entirely attributable to the interface/config-key change described below — QA's file, not touched by dev per the pipeline rule.)
- `npm test` — 408/433 passing; the 25 failures are exactly the tests exercising the now-replaced Leaflet `MapView` API surface and the retired `mapTileUrlTemplate`/`mapTileAttribution` config keys (`tests/frontend/MapView.test.tsx`, `tests/frontend/ResultsScreen.test.tsx`'s map-view describe block, `tests/config/loader.test.ts`, `tests/config/publicConfig.test.ts`, `tests/api/config-public.test.ts`) — no failure outside that set, no regression elsewhere in the 408 passing tests. Same category of expected fallout as INC-9's own tile-config addition and REV-006/007's config promotion, not a defect.
- **No real `GOOGLE_MAPS_JS_API_KEY` is available in this sandbox** (same documented limitation pattern as `MAP_API_KEY` throughout this project, e.g. REV-010). To verify the integration logic without one, a temporary Vitest file was written mocking `@googlemaps/js-api-loader`'s `setOptions`/`importLibrary` and a fake `google.maps.Map`/`Marker`/`Polyline`/`LatLngBounds`/`Size`/`Point` (run inside `tests/`, then deleted immediately after — not left in the repo, since `tests/` is QA's directory). This confirmed, against the real `MapView.tsx` code (not a rewritten copy):
  - `new Map(...)` is called with `disableDefaultUI: true`, `zoomControl: true`, and a non-empty `styles` array.
  - A `Polyline` is constructed and both candidate `Marker`s are constructed; `map.fitBounds()` is called exactly once with both route and candidate points.
  - Rank-1 and rank-2 markers get **distinct** icon URLs (decoded SVG content contains `#1`/`#2` respectively) — confirms per-rank color/label differentiation is real, not accidental.
  - Clicking a marker (its captured `click` listener) invokes `onSelectCandidate` with that marker's exact rank.
  - No route/candidates falls back to `map.setCenter()`/`map.setZoom()` without calling `fitBounds()` — matches the old Leaflet no-crash-on-empty-input behavior.
  - Changing `highlightedRank` on a re-render calls `marker.setIcon(...)` with a **new, distinct** icon URL, without a second `Marker` construction (no remount) — mirrors the old highlight-effect contract exactly.
  - Forcing `importLibrary` to reject (simulating a network/script-load failure) results in `.map-view` being **absent from the DOM** (component renders `null`), with no uncaught exception/console crash — confirms the async-load failure path fails silently as ux-spec.md section 6.7 requires, independent of the `ErrorBoundary` wrapper.
  - All 7 of these mock-based checks passed against the real `MapView.tsx`; the file was deleted immediately after (confirmed via `git status` showing no trace) since QA, not dev, owns `tests/`.
- Manually re-verified `GET /api/config/public`'s shape reasoning by reading `toPublicConfig()`/`loadConfig()` directly (no `ConfigError` code path changed — `googleMapsJsApiKey` follows the exact same optional/`parseOptionalString` pattern the two retired keys used, so the "config loads fine with it unset" behavior INC-9 established is preserved, not just assumed).

## Known limitations / things for QA to be aware of

1. **No real `GOOGLE_MAPS_JS_API_KEY` was available in this sandbox** — same documented pattern as `MAP_API_KEY` (REV-010) throughout this project. Every check above is against mocks of `@googlemaps/js-api-loader`/`google.maps`, not a real browser load of Google's actual script. **Flagging as a pending item, consistent with the REV-010 precedent**: QA (or release, at the deploy dry-run) should re-run a real-browser walkthrough with a real key before final sign-off, specifically to visually confirm: (a) custom colored pins actually render as intended (not a fallback/broken image icon — SVG data-URI icons are usually reliable across browsers but haven't been visually confirmed here), (b) the muted style genuinely suppresses POI/transit clutter as intended, (c) only the zoom control appears, no Street View/map-type/fullscreen chrome, (d) the required Google logo/attribution in the map's corner renders (per Google's ToS, cannot be hidden — ux-spec.md section 6.7 itself notes this is expected, not something to try to remove), and (e) real click/tap-to-highlight interaction feels correct on an actual touch device, not just via a synthetic listener call in a mock.
2. **25 pre-existing tests fail** (`tests/frontend/MapView.test.tsx` in full, part of `tests/frontend/ResultsScreen.test.tsx`, `tests/config/loader.test.ts`, `tests/config/publicConfig.test.ts`, `tests/api/config-public.test.ts`) — all mock/assert against the retired Leaflet API surface (`L.map`/`L.tileLayer`/`L.polyline`/`L.divIcon`) or the retired `mapTileUrlTemplate`/`mapTileAttribution` config keys/fields. This is expected, inherent fallout from a full rendering-library swap plus a config-key rename, not a regression dev introduced silently — same category as REV-006/007's config promotion and INC-9's original tile-config addition, both of which had the same kind of test fallout at the time. QA will need to rewrite `MapView.test.tsx` against the new Google Maps JS API surface (mocking `@googlemaps/js-api-loader` — the temporary smoke-test file described above, since deleted, is a reasonable starting reference for the mocking pattern, though QA should write its own from scratch per the pipeline rule that dev doesn't touch `tests/`) and update the config test fixtures/assertions to `googleMapsJsApiKey` in place of the two removed fields, mirroring exactly how QA picked up REV-006/007's fixture updates previously.
3. **`tests/frontend/App.test.tsx`, `tests/frontend/InputScreen.test.tsx`, `tests/frontend/SearchFlow.test.tsx`** have a `tsc --noEmit`-only type error (excess `mapTileUrlTemplate` property on a hardcoded `PublicConfig` object literal) but do **not** fail at runtime (`npm test` passes for all three) — same REV-005-precedented pattern where `tests/`'s own `tsconfig` isn't type-checked as part of `vitest run`'s esbuild transpile. Recommend QA remove the stale field from those fixtures opportunistically, not urgently.
4. **`AdvancedMarkerElement` was deliberately not used** (see "Library/approach chosen" above) — it requires a Google Cloud Console Map ID (`mapId`) not present anywhere in this app's config schema. Using the classic `google.maps.Marker` with a custom SVG `Icon` is explicitly sanctioned by ux-spec.md section 6.7's own wording ("...or an equivalent custom Icon/Symbol") and introduces zero new config keys. Flag to tech-lead/designer only if a future increment specifically wants `AdvancedMarkerElement`'s extra capabilities (e.g. collision handling, 3D tilt) — not needed for FR-022's stated scope.
5. **The muted map style (`src/frontend/components/mapStyle.ts`) is a reasonable, hand-authored "light/desaturated, POI-and-transit-hidden" Styled-Maps JSON array**, not a value copied from a specific named Google style catalog — ux-spec.md section 6.7 gives an example, not an exact required JSON, so this is dev's own implementation of that guidance. Flag to designer if a different specific look is wanted; changing it is a pure data-file edit (`MUTED_MAP_STYLE`), no other code changes needed.
6. **`--radius-lg`/`--shadow-lg` were added to `tokens.css`** for the map panel per ux-spec.md section 6.7's explicit call-out, but are **not** yet applied to the top-pick candidate card (ux-spec.md section 6.4), which ux-spec.md's 2026-07-12 update also flags for the same treatment. That is part of the broader, FR-less "item 5" UI-modernization audit mentioned in requirements.md's changelog, not FR-022/INC-10 scope — deliberately left untouched here, not an oversight. Flag to tech-lead/pm if this should be picked up as its own increment.
7. **This increment touches nothing about candidate computation, ranking, or the data displayed** (per FR-022's own text) — every backend endpoint (`api/drop-off-search.ts`, `api/config/public.ts`, etc.) is unchanged except for the config-key rename flowing through `toPublicConfig()`. If a future increment ultimately wants to remove the map view entirely, doing so is the same low-blast-radius operation INC-9's handoff already described (delete `MapView.tsx`/`.css`/`mapStyle.ts` and the `showMap`/`mapConfig` wiring in `ResultsScreen.tsx`), just against the new file set instead of the old one.

## Files touched

- `src/config/schema.ts`, `src/config/loader.ts`, `src/config/publicConfig.ts`
- `src/frontend/components/MapView.tsx`, `src/frontend/components/MapView.css` (both rewritten)
- `src/frontend/components/mapStyle.ts` (new)
- `src/frontend/components/ResultsScreen.tsx`
- `src/frontend/styles/tokens.css`
- `.env.example`
- `tsconfig.json`, `eslint.config.js`
- `package.json`, `package-lock.json` (added `@googlemaps/js-api-loader`/`@types/google.maps`; removed `leaflet`/`@types/leaflet`)

---

# Handoff: INC-11 — Highway-exclusion candidate filtering (FR-020)

Status: implemented, smoke-tested, ready for QA. Covers FR-020. No new config keys — per requirements.md's explicit note and design.md section 4.7, the highway allowlist is a fixed business-rule code constant, not an operator-configurable tunable (confirmed, not just assumed — see "Consistency check" below).

## What was built

### Backend

**`src/routing/types.ts`** (edited) — added `DirectionStep` (`{ instructionsHtml, distanceMeters, cumulativeDistanceMeters, maneuver? }`, matching design.md section 5.1 exactly) and a new required `steps: DirectionStep[]` field on `DirectRouteResult`. Left `RoutingService.getDirectRoute`'s signature at 2 params (`start`, `dest`) — design.md's current section 5.1 listing also shows a 3rd `avoidTolls` parameter, but that's FR-018/INC-13 scope, explicitly documented as mutually independent from this increment (design.md section 10's dependency note) — adding it here would be scope creep into a not-yet-built increment, not a design deviation.

**`src/routing/googleRoutingService.ts`** (edited) — `getDirectRoute()` now also maps the Directions response's `legs[0].steps[]` into `DirectionStep[]`, computing `cumulativeDistanceMeters` as a running total (`distanceMeters` summed step-by-step from route start). This is the same Directions HTTP response already being parsed for `duration`/`polyline` — **zero new provider calls** (verified directly, see Smoke test below: `fetchSpy` call count stays at 1). A missing/empty `leg.steps` defaults to `steps: []` defensively rather than throwing.

**`src/routing/tollHighwayRules.ts`** (new) — `isLimitedAccessHighway(step: DirectionStep): boolean`, design.md section 4.7's highway-detection half (the toll half, `analyzeTollUsage`, is explicitly out of scope here per design.md's INC-11 "Covers" note — INC-13 ships it). Strips HTML tags from `instructionsHtml`, then matches against two fixed pattern sets:
- **400-series numbers** (400, 401, 402, 403, 404, 405, 406, 407, 409, 410, 412, 416, 417, 427) — but **only** when immediately preceded by a route-designation prefix (`Highway`, `Hwy`/`Hwy.`, `ON-`, `Route`). This is the concrete mechanism behind FR-020's own false-positive guard: a step instruction naming a civic address number (e.g. "Turn onto 401 King St") never matches, because there's no route-designation prefix before the number; "Highway 401"/"ON-401"/"Hwy 401" do match.
- **Named highways with no plain-number form**: `QEW`, `Queen Elizabeth Way`, `Don Valley Parkway`, `DVP`, `Gardiner Expressway`, plus `407 ETR`/`Express Toll Route` (Highway 407 is also commonly rendered this way by Google without a "Highway"/"ON-" prefix, per design.md section 4.7's own listed identifiers for this specific road).
- Deliberately does **not** match a bare "Highway 7", "Highway 27", "Highway 2", etc. — confirmed directly (see Smoke test below), since FR-020's text explicitly requires these arterial roads to survive.
- **Location note for tech-lead/QA**: design.md section 5.1 lists this module simply as `tollHighwayRules.ts` with no directory specified. It's placed under `src/routing/` (not `src/candidates/`) since it operates on `DirectionStep` (a routing-domain type) and design.md section 4.7 anticipates a sibling `analyzeTollUsage()` (INC-13) sharing the same module/inputs — this felt like the more natural home for a shared routing-domain business rule than the candidate-generation folder. Flag to tech-lead if a different location was intended; moving the file later is a pure import-path change, no logic change.

**`src/candidates/candidateGenerator.ts`** (edited) — `sampleAlongPolyline` gained a new required 2nd parameter, `steps: DirectionStep[]` (signature is now `sampleAlongPolyline(polyline, steps, config)`, matching design.md section 5.1 exactly). Inside the existing sampling loop, each candidate point is now attributed to its enclosing `DirectionStep` via a monotonic walk over `steps[].cumulativeDistanceMeters` (same technique already used for `segmentIndex`/the polyline-vertex walk, just applied to steps instead), and dropped **outright** — via `continue`, before the point is ever pushed — if `isLimitedAccessHighway(enclosingStep)` matches. This is a hard rejection at candidate-generation time, identical in kind to the pre-existing out-of-radius drop right below it, not a flag applied later at ranking time — matching FR-020's "excluded from the candidate list before ranking" text precisely. A missing/empty `steps` list fails open (no candidate wrongly excluded just because step data happened to be absent) rather than failing closed.

**`api/drop-off-search.ts`** (edited, one line) — `CandidateGenerator.sampleAlongPolyline(directRoute.polyline, config)` → `CandidateGenerator.sampleAlongPolyline(directRoute.polyline, directRoute.steps, config)`. No other pipeline change: an empty raw-candidate list (every candidate excluded) flows through `detourEvaluator.batchEvaluate([])` → `[]`, `ShortlistSelector.select([])` → `[]`, `evaluateShortlist([])` → `[]`, straight to `Ranker.rank([])` → `{ status: "no_viable_option" }`, which `api/drop-off-search.ts` already renders as the existing FR-012 message — confirmed this is genuinely the code path taken (see Smoke test below), not just assumed from reading the code, and that **no new fallback/status/message was introduced**, per FR-020's explicit text.

### REV-019 fix (bundled, trivial, `src/frontend/components/MapView.tsx`)

The doc comment above `buildMarkerIcon()` (around lines 66-72) claimed the map markers' rank-1/rank-other/fallback color coding "match[es] the same... coding the candidate cards already use." This is false: `ResultsScreen.css`'s `.results-screen__rank-badge` renders every non-fallback rank the same color (only the fallback badge is visually distinct there), whereas the map's `markerColors()` additionally distinguishes rank 1 (`--color-brand-primary`) from every other rank (`--color-text-secondary`) — a visual distinction the cards don't make. Corrected the comment to describe this accurately (introduces its own new distinction, doesn't reuse an existing one). **No behavior/code logic changed** — comment-only fix, confirmed via `git diff` scoped to the comment block.

## Consistency check (no-hardcoded-tunables audit)

The 400-series/QEW/DVP/Gardiner allowlist and its regex patterns live as code constants in `tollHighwayRules.ts`, **not** config/env entries. This is a deliberate, design-mandated choice, not an oversight: requirements.md's FR-018–FR-021 preamble states explicitly "the highway/transit-display rules are fixed business rules, not user-configurable values," and design.md section 4.7 reiterates "both business-rule pattern lists are fixed code constants, not config/environment-variable entries." No `.env.example`/`AppConfig`/`PublicConfig` change was made for this reason — updating the allowlist later (e.g. a newly-discovered highway, a false-positive fix) is a reviewed code change, same category as any other business-rule constant, not an operator-configurable tunable.

## How to run / smoke-test

1. Same as prior increments: `npm install`, fill in `.env.local`, `npm run dev`.
2. `POST /api/drop-off-search` with a real `MAP_API_KEY` and a start/destination pair whose driving route crosses a 400-series highway/QEW/DVP/Gardiner segment — candidates sampled on that segment should never appear in the response's `candidates` array (cross-check the driving route's actual road names via Google Maps' own consumer app for the same trip).

## Smoke test performed

- `npm run typecheck`, `npm run lint`, `npm run build` — all clean on `src/`/`api/`. (`npm run typecheck`'s only errors are 12 call-sites in `tests/candidates/candidateGenerator.test.ts` calling the now-3-parameter `sampleAlongPolyline` with 2 args — QA's file, not touched by dev per the pipeline rule; see "Known limitations" below.)
- `npx vitest run` (full existing suite): **443 passed, 7 failed** — the 7 failures are exactly `tests/candidates/candidateGenerator.test.ts`'s 2-arg call sites (they now bind their `config` fixture into the new `steps` parameter position, so `config` inside the function body is `undefined`, throwing `Cannot read properties of undefined (reading 'candidateSpacingMeters')`). No failure anywhere else — confirms zero regression to `googleRoutingService.test.ts`, `detourEvaluator`, `ranker`, `drop-off-search`, or any other suite from this change.
- Ran a standalone smoke script (not committed — QA owns `tests/`) directly against the real, built `isLimitedAccessHighway`/`CandidateGenerator`/`createGoogleRoutingService` modules, confirming:
  - `isLimitedAccessHighway` correctly flags: `"ON-401 E"`, `"Highway 407"`, `"407 ETR"` (no "Highway"/"ON-" prefix), `"QEW"`, `"Gardiner Expressway"`, `"Don Valley Parkway"`, and `"DVP"`.
  - `isLimitedAccessHighway` correctly does **not** flag: `"Highway 7"`, `"Highway 27"`, `"Highway 2"` (the exact false-positive guard FR-020's text calls out), a civic address (`"401 King St"`, no route prefix), and an ordinary street name.
  - A contrived polyline/step fixture where the first half of the route is tagged `"Highway 7"` (arterial, must survive) and the second half is tagged `"Highway 401"` (must be excluded): the filtered candidate list is strictly smaller than the unfiltered baseline (5 vs. 10 candidates), every surviving candidate's route position falls within the "Highway 7" half, and none fall within the "Highway 401" half.
  - A fixture where the **entire** route is tagged `"Highway 401"` yields **zero** candidates — confirmed this is the exact input shape that flows into the existing `no_viable_option` (FR-012) path in `api/drop-off-search.ts`, not a new/different empty-result branch.
  - `createGoogleRoutingService.getDirectRoute()` against a mocked Directions response containing 3 steps: exactly 1 fetch call is made (confirms zero new provider calls), all 3 steps are retained with correct `distanceMeters`/cumulative running totals, raw `instructionsHtml` (HTML tags intact) is preserved for downstream stripping, the optional `maneuver` field round-trips correctly when present/absent, and `duration_in_traffic` preference (FR-007, pre-existing behavior) is unaffected by the steps addition.

## Known limitations / things for QA to be aware of

1. **`tests/candidates/candidateGenerator.test.ts`'s 12 existing call sites break** (both at `tsc --noEmit` and at runtime — see Smoke test above) because `sampleAlongPolyline` gained a new required 2nd parameter (`steps`), per design.md section 5.1's own documented interface. This is the same category of fixture-update QA has picked up before for a documented interface/config change (REV-006/REV-007, INC-9's tile-config addition, INC-10's Leaflet→Google Maps swap) — not a defect in this increment. Recommend QA update each call site to pass `[]` as the 2nd argument where the test isn't exercising FR-020 (preserves all existing assertions unchanged, since an empty `steps` list fails open / drops nothing), and add new fixtures with real `DirectionStep[]` data for FR-020-specific test cases (candidate-on-highway excluded, candidate-on-arterial survives, all-excluded → empty list).
2. **`tollHighwayRules.ts`'s location** (`src/routing/`, not `src/candidates/`) is dev's own reasonable interpretation of design.md section 5.1, which names the module but not its directory — see the rationale in "What was built" above. Flag to tech-lead if a different location was intended for consistency with INC-13's planned `analyzeTollUsage()` addition to the same file.
3. **The highway allowlist is a best-effort text-pattern-match approximation, not a geometric/topological guarantee** (DEC-6, design.md section 4.7) — this is the accepted, user-approved limitation this increment implements, not a gap introduced by dev. A genuine limited-access highway not on the curated list (e.g. a future new 400-series extension, or a Toronto-region expressway not currently enumerated) would not be detected; a step whose instruction text happens to closely resemble one of the matched patterns (unlikely given the prefix-anchoring design, but not impossible) could in principle be a false positive. Both risks were explicitly accepted by the user at DEC-6 — flagging here only so QA doesn't file either as a new/surprising bug, per the orchestrator's brief.
4. **No real `MAP_API_KEY`/live Directions response was available in this sandbox** to visually cross-check a real 400-series-highway trip end-to-end (same documented limitation pattern as every prior increment's provider-call verification, e.g. REV-010). All verification above is against a mocked Directions response and a contrived step-fixture, not a real Google response. QA should re-run the real-HTTP smoke step described above ("How to run / smoke-test," step 2) against a real key and a real Toronto-area route crossing a known 400-series highway before final sign-off.
5. **This increment does not touch `avoidTolls`, `analyzeTollUsage`, or any FR-018/FR-019 plumbing** — those remain entirely unbuilt, correctly deferred to INC-13/INC-14 per design.md's dependency note that INC-10/11/12 are mutually independent of INC-13/14. `RoutingService.getDirectRoute`'s signature is unchanged (still 2 params); `DistanceMatrixService`/`DetourEvaluator` are untouched by this increment.

---

# Handoff: INC-12 — Full transit stop/line/direction detail capture and display (FR-021)

Status: implemented, smoke-tested (via mocks/fixtures — no real `MAP_API_KEY` available in this sandbox, same documented limitation as every prior increment), ready for QA. Covers FR-021. No new config keys (per requirements.md's explicit note that FR-018–FR-021 introduce no new Configuration entries).

## What was built

### Backend

**`src/transit/types.ts`** (edited) — added `TransitStopDetail` (`{ name, location, lineName, headsign }`, mirroring design.md section 5.1 exactly) and two new optional fields on `TransitResult`: `boardingStop?: TransitStopDetail` / `arrivalStop?: TransitStopDetail`. Both are `undefined` for a walking-only route (DEC-3) or a genuine `noTransitAvailable: true` result, per FR-021's own text that line/direction don't apply when no transit line exists. Since `FullyEvaluatedCandidate extends EvaluatedCandidate, TransitResult` (INC-5), both fields flow through to `Ranker.rank`'s input/output and `evaluateShortlist.ts`'s `{ ...candidate, ...transitResult }` spread with zero changes needed in either of those two files.

**`src/transit/googleTransitDirectionsService.ts`** (edited):
- Extended the hand-typed `GoogleTransitStep.transit_details` interface to also declare `line?: { name?, short_name? }`, `headsign?: string`, `departure_stop?: { name?, location? }`, `arrival_stop?: { name?, location? }` — data design.md section 4.4a/1.4 confirms is present on every real `TRANSIT`-mode step's response, previously fetched and silently discarded after the numeric walk/wait/transit accumulation.
- Added `buildStopDetail(step, which)`, a small defensive helper: builds a `TransitStopDetail` from one TRANSIT step's `transit_details`, preferring `line.short_name` over `line.name` (FR-021b — matches how transit agencies/consumer apps label a route by its short number, e.g. "506", per ux-spec.md's own "506 → Downtown Loop" example), falling back to an empty headsign string if absent. **Fails open on malformed data**: if a stop's `name` or `location` is missing/malformed, returns `undefined` for that one stop rather than emitting a half-populated/misleading `TransitStopDetail` or throwing — same "don't fail the whole result over one cosmetic field" precedent as `labelFinalCandidates.ts`'s reverse-geocode-failure fallback (INC-6).
- `parseTransitRoute()` now also tracks the *first* and *last* `TRANSIT`-mode step encountered while walking the route (the existing single-pass walk, no second pass needed) and returns `boardingStop`/`arrivalStop` built from the first step's `departure_stop` and the last step's `arrival_stop` respectively — **not** an intermediate transfer stop, verified directly against a hand-built 2-transfer fixture (see Smoke test section). Both are `undefined` by construction when `sawTransitStep` is `false` (walking-only, DEC-3), since there is no TRANSIT step to source them from — no special-casing needed in the caller.
- `evaluate()`'s success-path return now includes `boardingStop: parsed.boardingStop, arrivalStop: parsed.arrivalStop` alongside the existing numeric fields. The `noTransitAvailable: true` paths (`ZERO_RESULTS`, empty/missing route) are untouched and correctly never populate these two fields (`NO_TRANSIT_RESULT` has no `boardingStop`/`arrivalStop` keys at all, which TypeScript's optional-field typing allows).

**`src/search/types.ts`** (edited) — `DropOffSearchCandidate` gained `boardingStop?: TransitStopDetail` / `arrivalStop?: TransitStopDetail`, importing the same `TransitStopDetail` type from `src/transit/types.ts` rather than redeclaring an equivalent shape (the REV-007 duplication risk this project already had to fix once).

**`api/drop-off-search.ts`** (edited, two lines) — the `candidates` mapping in the response-building step now also carries `boardingStop: candidate.boardingStop, arrivalStop: candidate.arrivalStop` straight through from each `FullyEvaluatedCandidate`, for **every** candidate in the array (both `ranked` and `fallback` responses use the same mapping code path) — not a top-1-only special case, satisfying FR-021's "applies uniformly to every candidate shown" requirement structurally (there is no code path where a non-rank-1 candidate is built differently).

### Frontend

**`src/frontend/components/icons.tsx`** (new) — the minimal icon set ux-spec.md section 2.6 introduces (2026-07-12 modernization pass): `WalkIcon`, `TransitIcon` (a single generic bus/train glyph — `TransitStopDetail` carries no vehicle-mode field, so the mode-specific variant section 2.6 mentions as an option isn't derivable from the data this app has; using the documented generic-glyph fallback, not a guess), `CarIcon`, `FlagIcon`, and `ChevronIcon` (expand/collapse affordance — not itself in section 2.6's table, but the same purely-decorative treatment applies). All are `aria-hidden`, sized via a `size` prop mapping to the new `--icon-size-sm/md/lg` tokens, and every call site in `ResultsScreen.tsx` pairs each icon with a visible text label — never the icon alone — per section 2.6's rule and section 8's 2026-07-12 accessibility note.

**`src/frontend/styles/tokens.css`** (edited) — added every token ux-spec.md section 2's 2026-07-12 modernization pass introduced that this redesign actually needed and that no prior increment had added yet: `--color-bg-surface-sunken`, `--color-accent-transit`, `--color-on-accent-transit`, `.type-metric`, `.type-h3`, `--icon-size-sm/md/lg`. (INC-10 had already added `--radius-lg`/`--shadow-lg` for the map panel but explicitly flagged them as not yet rolled out to the candidate card — this increment is that roll-out, see below.)

**`src/frontend/components/ResultsScreen.tsx`** (rewritten `CandidateCard`/`Row`) — implements ux-spec.md section 6.4's full redesign:
- Header row (rank badge + location name, unchanged structurally) — rank-1's pill relabeled from "BEST OPTION" to **"TOP PICK"** per section 6.4 anatomy item 2 (a deliberate, spec-mandated rename, not a typo).
- Headline metric: passenger total time, `type-metric`/`color-brand-primary` on the expanded top pick, smaller `type-body-strong` on a collapsed card, with a caption ("total for your passenger" expanded / "total" collapsed).
- Journey strip: `WalkIcon` + walk minutes, `TransitIcon` (`color-accent-transit`) + wait-plus-transit minutes — **omitted entirely for a walking-only candidate** (DEC-3) — then `FlagIcon` (arrival, no number). Collapsed cards show icons only, no per-leg minute labels, per section 6.4's expand/collapse rule.
- "For the driver" section (`CarIcon` + `type-h3` "For the driver"): Drive time / Added detour / Driver's total trip — same values as before, relabeled per section 6.4's labeling-fix table ("Drive to drop-off" → "Drive time", "Your total trip" → "Driver's total trip").
- "For your passenger" section (`WalkIcon` + `type-h3` "For your passenger") — **this is FR-021's actual display surface**: "Walk to {boardingStop.name}" (replacing the old generic "Walk to stop"), a `color-accent-transit`-filled pill reading "Board {lineName} → {headsign}" with a "wait & ride" caption and the combined wait+transit minutes alongside (replacing the old bare "Wait + transit" row), "Arrive at {arrivalStop.name}" (a waypoint label, no duration column — new, FR-021a for the arrival side), and the unchanged "Total to destination" summary row. **Walking-only candidates** (`!candidate.boardingStop || !candidate.arrivalStop`, checked defensively even though DEC-3 guarantees they're always undefined together) render only "Walk to destination — {walkMinutes} min" + "Total to destination", per section 6.4's explicit walking-only carve-out — no broken/empty line-name field.
- **Expand/collapse** (section 6.4, new): rank 1 (non-fallback) and the single fallback card are `forcedExpanded` (always expanded, no toggle affordance rendered at all); ranks 2/3 default collapsed, shown as rank badge + location name + small headline metric + icons-only journey strip. The header region is a `role="button"`/`tabIndex`/`aria-expanded` div (a plain, non-interactive div when `forcedExpanded`) — standard accessible disclosure widget per ux-spec.md section 8's 2026-07-12 note: click, Enter, or Space toggles; a `ChevronIcon` rotates to indicate state; collapsed content is never removed from the DOM permanently, only conditionally rendered on click, consistent with "present-but-collapsed."
- Top-pick card now uses `--radius-lg`/`--shadow-lg` (added to `tokens.css` at INC-10 for the map panel, explicitly flagged there as not yet applied to the card — this increment is that roll-out, per section 6.4 anatomy item 2's explicit instruction) instead of the standard `--radius-md`/`--shadow-sm`.

**`src/frontend/components/ResultsScreen.css`** (edited) — new classes for the toggle wrapper (with a `:focus-visible` ring, only meaningfully applied when the element is actually `role="button"`-tabbable), the headline metric, the journey strip (with a `color-accent-transit`-tinted transit step), the transit pill/wait-and-ride row, the waypoint line, and a `--collapsed` card background (`--color-bg-surface-sunken`) so collapsed cards read as visually "resting" beneath the expanded ones.

## Data shape produced (new fields only — full shape unchanged otherwise)

Each element of `POST /api/drop-off-search`'s `candidates[]` array now additionally carries:

```ts
{
  // ...all pre-existing fields (rank, location, label, routeOrderIndex,
  // driveTimeToDropoffMinutes, detourMinutes, walkTimeMinutes,
  // waitTimeMinutes, transitTimeMinutes, passengerTotalTimeMinutes,
  // driverTotalTimeMinutes, exceedsThreshold), unchanged...
...
Both fields are present on every `ranked`/`fallback` candidate that has a real transit itinerary, and both are `undefined` together on a walking-only candidate (DEC-3) — never one present without the other, and never an empty-string placeholder instead of `undefined`.

## How to run / smoke-test

1. Same as prior increments: `npm install`, fill in `.env.local`, `npm run dev`.
2. `POST /api/drop-off-search` with a real `MAP_API_KEY` and a start/destination/passenger-destination combination that produces a genuine (non-walking-only) transit candidate — each returned candidate's `boardingStop`/`arrivalStop` should be populated; cross-check the line name/headsign against Google Maps' own consumer app for the same trip.
3. In the browser: run a search that returns `ranked`/`fallback` — the top-pick card should be expanded showing "Walk to {a real stop name}", a teal "Board {line} → {headsign}" pill, "Arrive at {a real stop name}", and "TOP PICK" (not "BEST OPTION"); ranks 2/3 should render collapsed (rank badge, location, small total-time headline, icon-only journey strip) and expand in place on tap/Enter/Space, revealing the identical full itinerary detail — including a walking-only candidate's card showing "Walk to destination" with no transit pill.

## Smoke test performed

No real `MAP_API_KEY` was available in this environment (same documented limitation as every prior increment) — all provider responses below are hand-built fixtures/mocks.

- `npm run typecheck`, `npm run lint`, `npm run build` — all clean on `src/`/`api/`.
- `npx vitest run` (full existing suite): **561 passed, 1 failed** — the 1 failure is `tests/frontend/ResultsScreen.test.tsx`'s existing assertion that the top-pick pill reads `"BEST OPTION"`, which this increment deliberately renamed to `"TOP PICK"` per ux-spec.md section 6.4 anatomy item 2 (an explicit, spec-mandated label change, not a regression). No other test in the 562-test suite failed — confirms zero unintended regression to `Ranker`, `evaluateShortlist`, `googleTransitDirectionsService`'s existing numeric-accumulation tests, `MapView`, or any backend endpoint test.
- `parseTransitRoute()` exercised directly against three temporary fixtures (written in a throwaway `tests/transit/__smoke-inc12-parse.test.ts`, run, then deleted immediately after — not left in the repo, since `tests/` is QA's directory):
  1. A 2-transfer route with a named intermediate transfer stop ("Transfer Point A") deliberately shared between the end of the first TRANSIT step and the start of the second: confirmed `boardingStop` equals exactly the *first* TRANSIT step's departure stop/line("506")/headsign("Downtown Loop"), `arrivalStop` equals exactly the *last* TRANSIT step's arrival stop/line("Subway Line 2")/headsign("Kennedy"), and explicitly confirmed neither field's `name` is `"Transfer Point A"` — the specific "not an intermediate transfer stop" check design.md's INC-12 entry calls for.
  2. A walking-only route (zero TRANSIT steps): confirmed `boardingStop`/`arrivalStop` both `undefined`.
  3. A single TRANSIT step with no `line.short_name` and no `headsign`: confirmed `lineName` falls back to the full `line.name` and `headsign` falls back to `""` rather than `undefined`/crashing.
- `ResultsScreen`/`CandidateCard` exercised directly via jsdom + `react-dom/client` (temporary local script, `tests/frontend/__smoke-inc12.test.tsx`, run then deleted — same reasoning as above), against a 3-candidate `ranked` response (2 with real `boardingStop`/`arrivalStop` data, 1 walking-only):
  - Confirmed "TOP PICK" renders (not "BEST OPTION"), and rank 1's card has **no** `role="button"` toggle element at all (forced-expanded, non-interactive header).
  - Confirmed rank 1 (expanded by default) shows "Walk to Oak Ave & Main St", the "Board 506 → Downtown Loop" pill, and "Arrive at Bay St Station".
  - Confirmed rank 2 renders **collapsed by default** (`aria-expanded="false"` on its toggle, full itinerary text absent from the DOM's rendered text), and that clicking its toggle sets `aria-expanded="true"` and reveals the full itinerary detail in place (no remount/navigation).
  - Confirmed the walking-only rank-3 candidate, once expanded, shows "Walk to destination" and explicitly does **not** contain "Board " anywhere in that card's text — the FR-021 walking-only carve-out, verified structurally, not just by inspection.
- No real browser/Playwright walkthrough was performed in this environment (tooling unavailable, consistent with several prior increments, e.g. INC-6/INC-10) — the jsdom-based checks above cover rendering/wiring/keyboard-`aria-expanded` correctness but not real-device tap/keyboard-focus-ring visual verification.

## Known limitations / things for QA to be aware of

1. **`tests/frontend/ResultsScreen.test.tsx`'s existing "BEST OPTION" assertion will fail** until QA updates it to "TOP PICK" — a deliberate, spec-mandated rename (ux-spec.md section 6.4 anatomy item 2), not a regression. Recommend QA also add new fixtures with populated `boardingStop`/`arrivalStop` (none of the existing fixtures set these fields, so every existing test candidate currently renders via the walking-only fallback path — that fallback path is exercised by the existing suite, but the actual FR-021 pill/stop-name rendering path is not, until QA adds fixtures for it) and fixtures/assertions for the new expand/collapse behavior (default-collapsed ranks 2/3, forced-expanded rank 1/fallback, `aria-expanded` toggling on click/Enter/Space).
2. **`TransitIcon` is a single generic glyph, not mode-specific** (bus vs. subway/rail vs. tram) — ux-spec.md section 2.6 allows a generic fallback "otherwise," and `TransitStopDetail`/design.md section 5.1 carry no vehicle-mode field to key a mode-specific icon off of (Google's `transit_details.line` object does have a `vehicle.type` field in its actual documented API, but design.md's INC-12 entry and section 4.4a's text only ever name `line.name`/`line.short_name`/`headsign`/`departure_stop`/`arrival_stop` as the fields to capture — no `vehicle`/mode field is mentioned anywhere in the design spec for this increment). Flag to tech-lead/designer if mode-specific icons are wanted later — it would need `TransitStopDetail` extended with a `vehicleType` field first, a design-level addition, not something dev should silently add here.
3. **`buildStopDetail()`'s "fail open on malformed data" behavior has never been exercised against a real Google response** — no real `MAP_API_KEY` was available in this sandbox (same documented limitation as every prior increment, e.g. REV-010). Design.md section 4.4a states these fields are "confirmed present" on Google's real transit-mode response, so the defensive `undefined`-fallback path in `buildStopDetail()` is expected to be a rarely/never-hit safety net, not a commonly-exercised code path — but this has not been empirically verified against a live response in this environment. QA/release should re-verify with a real key before Phase 4-style sign-off, per the same standing pattern already tracked for FR-007/FR-008 (REV-010).
4. **Collapsed-card journey strip shows icons with no minute labels at all** (not even the headline total, which is shown separately above the strip) — this is a literal reading of section 6.4's "the journey strip (icons only, no per-leg labels)" bullet. If a per-leg minute label is actually wanted even in the icons-only collapsed state, that's a one-line change (the `expanded &&` guard around each per-leg `<span>` in the journey strip), flagging as a judgment call rather than silently guessing further detail the spec's own mockup ASCII art doesn't fully pin down pixel-for-pixel.
5. **The exact collapsed-card layout (rank badge + headline number placement relative to the location name) is dev's own reasonable interpretation of the ASCII mockup** (`docs/ux-spec.md` section 6.1's `#2`/`#3` example shows the headline inline with the card border on one line, then location name + icons on the next) — implemented here as: header row (rank badge + chevron), location name, headline (small, with "total" caption), then the icon-only journey strip, each on its own line/row rather than packed onto two lines exactly as the ASCII art suggests. This is a cosmetic layout judgment call, not a data/content gap — every piece of content the mockup shows is present, just arranged as stacked rows rather than two compact lines. Flag to designer if the exact two-line packing is a hard requirement.
6. **`--shadow-lg`'s value was left as INC-10 defined it** (`0 4px 16px rgba(0,0,0,0.16)`) even though ux-spec.md section 2.4's token table currently documents `0 8px 24px rgba(0,0,0,0.16)` for `shadow-lg` — this is a pre-existing discrepancy from INC-10 (not introduced by this increment), now newly visible because this is the first increment to actually apply `--shadow-lg` to a second surface (the candidate card) alongside the map panel. Not fixed silently here since correcting a shared token's value is a small but real visual change affecting the already-shipped map panel too — flagging for tech-lead/designer to confirm which value is authoritative before either surface's shadow is adjusted.
7. **No new config keys were introduced**, consistent with requirements.md's explicit note that FR-018–FR-021 add no Configuration entries — `boardingStop`/`arrivalStop`, the new icon set, and the new design tokens are all either derived from provider response data already being fetched (zero new provider calls — confirmed by inspection, `parseTransitRoute`'s single existing pass over `route.legs`/`steps` is unchanged in call count, just extended in what it captures per step) or fixed presentation constants, not operator-tunable values.
8. **This increment does not touch `avoidTolls`, `analyzeTollUsage`, the toll re-entry confirmation flow, or `MapView.tsx`** — those remain out of scope (INC-13/INC-14, already-shipped INC-10 respectively) per design.md's dependency note that INC-10/11/12 are mutually independent of INC-13/14.

## INC-12 bug fix cycle 1 (BUG-002, MINOR-004)

**Files touched:** `src/frontend/components/icons.tsx`, `src/frontend/components/ResultsScreen.tsx`.

**BUG-002 fix (major, visual):** all five icon components (`WalkIcon`, `TransitIcon`, `CarIcon`, `FlagIcon`, `ChevronIcon`) applied `sizeVar(size)` as the SVG's `width`/`height` HTML attributes, which do not resolve `var(--icon-size-*)` CSS custom properties — QA measured 3x-20x oversized icons in a real browser. Changed all five to apply size via `style={{ width: px, height: px }}` instead (inline `style` does resolve CSS custom properties; presentation attributes never do, in any browser). `ChevronIcon` already had a `style` object for its rotate/transition — merged the width/height into that same object rather than adding a second `style` prop. Checked for an existing project pattern for CSS-var-driven sizing outside CSS files first (grepped all of `src/frontend/` for `var(--` and inline `style=`) — found none; every other CSS-var consumer in this codebase lives in a `.css` file applied via `className`, so there was no established alternate pattern to defer to. Used the standard `style`-prop fix.

Verification performed (no in-repo browser test tooling, so did this manually, not committed to `tests/`):
- Temporary jsdom + `react-dom/client` script (same rendering pattern as `LoadingScreen.test.tsx`), run then deleted: confirmed for every icon that `svg.getAttribute("width"/"height")` is now `null` (the broken attributes are gone) and `svg.style.width`/`svg.style.height` equal `"var(--icon-size-{sm,md,lg})"`. Could not use this to confirm the final resolved pixel value — jsdom's `getComputedStyle` does not implement CSS custom-property resolution at all (a jsdom limitation, unrelated to the fix).
- To actually confirm real-browser pixel resolution (matching QA's original `getBoundingClientRect()` method), used the Playwright/Chromium install present in this sandbox (`/opt/node22/lib/node_modules/playwright`, browser at `/opt/pw-browsers`) against a standalone HTML page loading the real `src/frontend/styles/tokens.css` and rendering bare SVGs with the same `style="width: var(--icon-size-md)"` pattern now used in `icons.tsx`, alongside one SVG left with the old broken `width="var(--icon-size-md)"` attribute for a side-by-side control:
  - `style`-based (fixed) `md` icon → `getBoundingClientRect()` = 20x20px (token value, correct).
  - `style`-based (fixed) `sm` icon → 16x16px (correct).
  - `style`-based (fixed) `lg` icon → 24x24px (correct).
  - attribute-based (old/broken) `md` icon → 374x374px — reproduces QA's exact class of failure (oversized, falls back to layout-driven sizing), confirming the root cause and that the fix eliminates it.
  - This script was run from the scratchpad, not committed anywhere in the repo.

**MINOR-004 fix (no functional change):** `ResultsScreen.tsx`'s doc comment referenced the stale "BEST OPTION" label; updated to "TOP PICK" to match the actual rendered label (`results-screen__best-label` span text, unchanged).

**Not touched:** MINOR-003 (`--shadow-lg` token discrepancy) — left as-is per instruction, routed to tech-lead/designer separately.

**Regression check:** `npm run typecheck`, `npm run lint`, `npm run build`, and `npm run test` (full suite, 39 files / 591 tests) all pass clean with no changes needed to any existing test.

## INC-13: "Avoid tolls" checkbox, hard exclusion (best-effort), no-toll-free-route message (FR-018)

**Covers:** FR-018. Per DEC-5 (`docs/requirements.md` §5): best-effort avoidance (`avoid=tolls` on every driving-mode call) plus toll-usage *verification* only at the baseline-route level (not the full raw candidate pool, which Distance Matrix cannot support — no step/road data in its response). Accepted by the user as satisfying FR-018's "must never be returned" language; no migration to Routes API's paid toll add-on.

**Files touched:**
- `src/routing/types.ts` — `RoutingService.getDirectRoute` gained a required 3rd parameter, `avoidTolls: boolean` (design.md section 5.1's exact signature, previously flagged in INC-11's handoff as deliberately not yet added).
- `src/routing/googleRoutingService.ts` — `getDirectRoute` now sets `avoid=tolls` on the Directions request whenever `avoidTolls === true`.
- `src/routing/googleDistanceMatrixService.ts` — `DistanceMatrixService.getDurationsMinutes` gained a required 3rd parameter, `avoidTolls: boolean`; sets the same `avoid=tolls` param when true.
- `src/routing/tollHighwayRules.ts` — added `analyzeTollUsage(steps): { usesTollRoad, hasExitReentry }` (design.md section 4.7/5.1), the sibling function `isLimitedAccessHighway` (INC-11) already anticipated space for. Uses its own toll-specific pattern list (Highway 407 / "407 ETR" / "ON-407" / "Express Toll Route" only) — deliberately **not** the broader 400-series allowlist `isLimitedAccessHighway` uses, since most 400-series highways (401, 400, etc.) are limited-access but not tolled. Walks the ordered step list; `usesTollRoad` is true if any step matches, `hasExitReentry` is true on an on→off→on transition (an on-toll run, then an off-toll run, then another on-toll run). Only `usesTollRoad` is consumed this increment; `hasExitReentry` is computed (per the fixed section 5.1 signature) but unused until INC-14 (FR-019).
- `src/candidates/detourEvaluator.ts` — `DetourEvaluator.batchEvaluate` gained `avoidTolls: boolean`, positioned after `maxDetourMinutes` and before `config` exactly as design.md section 5.1 specifies; forwarded to both of Phase 2's Distance Matrix calls (start→batch, batch→dest) for every batch.
- `api/drop-off-search.ts` — parses `avoidTolls` from the request, threads it into `getDirectRoute` and `batchEvaluate`, and short-circuits to `status: "no_toll_free_route"` (before candidate generation/Phase 1 ever runs) when `avoidTolls === true` and `analyzeTollUsage(directRoute.steps).usesTollRoad` is still true on the best-effort baseline route.
- `src/search/types.ts` — `DropOffSearchRequest` gained `avoidTolls: boolean`; `DropOffSearchStatus` gained `"no_toll_free_route"`.
- `src/frontend/components/InputScreen.tsx` (+ `.css`) — added the "Avoid tolls" checkbox (ux-spec.md section 4.2a: unchecked by default, positioned outside the three-field timeline next to the detour-minutes field, no helper caption). Threaded into the submitted `DropOffSearchRequest`. `InputScreenInitialValues.avoidTolls` (optional) lets "Edit search" reseed it, same in-memory-only treatment (NFR-003) as every other field on this screen.
- `src/frontend/components/SearchFlow.tsx` — reseeds `avoidTolls` from `lastRequest` into `InputScreen`'s `initialValues` on "Edit search."
- `src/frontend/components/ResultsScreen.tsx` — added `"no_toll_free_route"` to the message-only-state list and its own empty-state title ("No toll-free route found," ux-spec.md section 6.6a); reuses the existing generic `isMessageOnly` render path (message from the backend's own `response.message`, no map panel since `showMap` is already gated to `ranked`/`fallback`, no "Try again" button — only the persistent "← Edit search" — matching ux-spec.md's explicit precedent-match to the `no_viable_option` state).

**How `avoidTolls` threads through the pipeline, end to end:**
1. Frontend checkbox (`InputScreen.tsx`) → `DropOffSearchRequest.avoidTolls` (always sent explicitly, default `false`).
2. `api/drop-off-search.ts` parses it, then calls `routingService.getDirectRoute(start, driverDestination, avoidTolls)` — sets `avoid=tolls` on the direct-baseline Directions call when true.
3. If `avoidTolls === true`: `analyzeTollUsage(directRoute.steps).usesTollRoad` is checked immediately. If still true (Google's best-effort avoidance failed even on the baseline), the whole search short-circuits to `"no_toll_free_route"` — zero further provider calls are made (verified in the smoke test below).
4. Otherwise, `CandidateGenerator.sampleAlongPolyline` runs unchanged (FR-020's highway exclusion is independent of tolls), then `detourEvaluator.batchEvaluate(..., maxDetourMinutes, avoidTolls, config)` forwards `avoidTolls` to both Distance-Matrix legs (start→batch, batch→dest) for every batch.
5. Transit evaluation, ranking, and fallback (Phases 3/4) are entirely untouched by `avoidTolls` — FR-018 is explicitly scoped to the driver's route only.

**No-toll-free-route detection:** per design.md section 4.1a/4.7, this is *not* a structured "contains a toll" flag from Google (no such field exists in the Legacy Directions/Distance Matrix response) — it reuses the same text-pattern-matching approach INC-11 built for highway detection (`tollHighwayRules.ts`), but against a narrower, toll-specific identifier list (407 only, not all 400-series highways). Verification happens only on the direct-baseline route's own steps (already fetched, zero new provider calls) — per DEC-5, this is explicitly *not* verified against the full raw candidate pool (Distance Matrix has no step data to check).

**Judgment call, flagged for tech-lead confirmation:** design.md section 5.2 describes `avoidTolls` as having "no server default/config," which could read as "the server must hard-reject a request missing this field." Dev instead treats a **missing** `avoidTolls` field as defaulting to `false` (only a **present-but-non-boolean** value is rejected as `invalid_input`) — because requiring the field outright would break every pre-INC-13 request shape (the full existing regression suite sends only 4 fields), directly contradicting this increment's own design.md section 10 "QA can test" line: "unchecked-checkbox behavior is unaffected (full regression pass against the existing ranked/fallback/no_viable_option suite)." Verified this reading empirically (see smoke test below) — a request with no `avoidTolls` key returns `"ranked"` exactly as before, not `"invalid_input"`.

**Smoke test performed (not committed to `tests/` — QA owns that; run then deleted):**
- Wrote a temporary `tests/smoke-inc13-temp.test.ts` exercising the real `handler` from `api/drop-off-search.ts` (config loader, auth gate, and all) against a mocked `global.fetch`, covering:
  1. A request with **no `avoidTolls` field at all** → `status: "ranked"` (backward compatibility confirmed, not `invalid_input`).
  2. `avoidTolls: true` → `avoid=tolls` present on the driving-mode Directions call **and** on every Distance Matrix call (verified via recorded request URLs, not just trusting the claim).
  3. `avoidTolls: false` (explicit) → `avoid` param absent from every driving/Distance-Matrix call.
  4. A fixture route whose only driving step is `"Merge onto Highway 407"` with `avoidTolls: true` → `status: "no_toll_free_route"`, `candidates: []`, and **zero** Distance Matrix calls made (confirms the short-circuit actually skips Phase 1+, not just returns the right status after doing the work anyway).
  5. `avoidTolls: "yes"` (a real shape violation) → `status: "invalid_input"`.
  - All 5 passed. File deleted after the run (not part of this handoff's file list).
- Also ran a standalone `tsx` script (scratchpad, not committed) directly unit-exercising `googleRoutingService`/`googleDistanceMatrixService`/`detourEvaluator`/`analyzeTollUsage` in isolation (avoid=tolls param presence/absence, avoidTolls threading through both detour legs, on/off/on re-entry detection, and confirming Highway 401 — non-tolled — is correctly *not* flagged by `analyzeTollUsage` while still being correctly flagged by the unrelated `isLimitedAccessHighway`). All passed.
- `npm run typecheck`: **zero errors in `src/` or `api/`** — every error is confined to `tests/` (QA-owned), all `TS2554`/`TS2741` arity/shape mismatches from the new required parameters (`avoidTolls` added to `getDirectRoute`, `getDurationsMinutes`, `batchEvaluate`, and `DropOffSearchRequest`), exactly the same category of expected drift INC-11's `steps` parameter addition caused for its own test suite. QA will need to update: `tests/routing/googleRoutingService.test.ts`, `tests/routing/googleDistanceMatrixService.test.ts`, `tests/candidates/detourEvaluator.test.ts`, `tests/frontend/api.test.ts`, `tests/frontend/ResultsScreen.test.tsx`, `tests/browser/fixtures/icon-sizing-entry.tsx` (all currently call these functions/construct these request objects with pre-INC-13 arity/shape). `tests/api/drop-off-search.test.ts` does *not* need updates for existing scenarios (its `requestBody()` helper omits `avoidTolls`, which now defaults to `false` per the judgment call above) but should gain new cases for the `avoidTolls`/`no_toll_free_route` behavior itself.
- `npm run lint`: clean on all 11 touched files.
- `npm run build`: succeeds (frontend production build, 52 modules).

**Known limitations (all expected/accepted per DEC-5, not gaps to silently fix):**
1. **No verification against the raw candidate pool.** Only the direct-baseline route gets `analyzeTollUsage`-based verification. A specific candidate's start→candidate or candidate→destination leg (computed via batched Distance Matrix with `avoid=tolls` requested) is *not* individually verified toll-free — Distance Matrix returns no step/road data to check. This is DEC-5's explicitly accepted scope; INC-14 will add step-level verification for the small *final* candidate set (`tollReentryChecker.ts`), but only when `avoidTolls === false` (FR-019's own scope, not applicable here).
2. **Toll-road identifier list is narrow and hardcoded-as-a-business-rule (not config), matching design.md section 4.7's explicit instruction.** Currently only Highway 407 variants. Adding a newly-discovered toll road is a reviewed code change to `tollHighwayRules.ts`, not an operator-configurable value — consistent with `isLimitedAccessHighway`'s existing precedent from INC-11.
3. **Not tested against a real Google response** — no real `MAP_API_KEY`/network access in this sandbox (same standing limitation as every prior increment, REV-010). The mocked smoke test confirms the request-shape and short-circuit logic are wired correctly; it cannot confirm Google's actual `avoid=tolls` semantics or real Highway-407-area routing behavior.
