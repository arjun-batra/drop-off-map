...
No new Configuration entries are introduced by FR-018 through FR-021 — the toll checkbox and re-entry question are per-session user inputs (like FR-002's detour threshold), not operator-controlled tunables, and the highway/transit-display rules are fixed business rules, not user-configurable values. FR-022 does introduce a Configuration consideration (the Google Maps JavaScript API key/credential used for map rendering); tech-lead should confirm during design whether this reuses the existing `MAP_API_KEY` entry or requires a distinct key, and pm will update the Configuration table accordingly once that is settled.
