...
### Toll Avoidance & Highway Exclusion — Change Request 2026-07-12 (STATUS: DRAFT, PENDING USER APPROVAL)

The following FR-018 through FR-021 are new/changed requirements arising from a post-launch change request. They are drafted per the user's answers to pm's clarifying questions and are **not yet approved** — see "Approval needed" note at the end of this subsection and Changelog entry dated 2026-07-12. Do not begin design/implementation work against these until the user approves.
...
- **FR-018**: The system shall provide an "avoid tolls" checkbox as a per-session input control (default unchecked/tolls-allowed, no persistence across sessions per NFR-003). When checked, every driving-route calculation performed for the driver — the direct baseline route (FR-006b's "no stop" baseline), the start-to-candidate leg, and the candidate-to-destination leg, for every candidate evaluated — shall exclude toll roads entirely. This is a hard exclusion, not a preference, cost penalty, or soft ranking factor: a driver-side route that uses any toll road must never be returned or offered when this checkbox is checked. This requirement applies only to the driver's route calculations; it has no effect on the passenger's transit itinerary calculation (FR-006c/d).
  - **Open question (OQ-9, blocking approval)**: this FR does not yet specify what the system should do if, for a given start/destination pair, no toll-free driving route exists at all (i.e., every possible route includes a toll road). Options include: (a) fall back to a toll-inclusive route with a visible warning (mirroring FR-011's detour-threshold fallback pattern), (b) treat it the same as FR-012's "no viable option" case, or (c) some other behavior. This must be resolved by the user before FR-018 can be finalized.

- **FR-019**: When the "avoid tolls" checkbox (FR-018) is unchecked (tolls allowed) and the driver's calculated route for a given candidate includes a toll-road segment that the route exits and later re-enters within the same trip, the system shall present the user with a question asking whether this exit/re-entry pattern is acceptable. This question shall only be presented when both conditions hold: (a) FR-018's checkbox is unchecked, and (b) the calculated route actually contains a toll-road exit followed by a later re-entry of a toll road within the same trip. When FR-018's checkbox is checked (tolls avoided entirely), this question shall never be presented, since no toll segment exists to exit or re-enter.
  - **Open question (OQ-10, blocking approval)**: the user's answer confirms this must be presented as a question to the user rather than a silent filter, but does not yet specify what the system does with the answer. Does answering "not acceptable" (1) exclude that specific candidate/route from the results, (2) trigger recalculation of that candidate's route to avoid the exit/re-entry pattern specifically (while still allowing tolls elsewhere), (3) simply record/display the concern without changing the candidate list, or (4) something else? This must be resolved by the user before FR-019 can be finalized — its testable behavior is currently undefined.
...
**Approval needed from the user**: FR-018 through FR-021 as drafted above, AND resolution of OQ-9 (no-toll-free-route fallback behavior) and OQ-10 (consequence of the toll re-entry question's answer) before FR-018/FR-019 can be marked final. FR-020 and FR-021 have no outstanding open questions and are ready for approval as drafted.

No new Configuration entries are introduced by FR-018 through FR-021 — the toll checkbox and re-entry question are per-session user inputs (like FR-002's detour threshold), not operator-controlled tunables, and the highway/transit-display rules are fixed business rules, not user-configurable values.

**Item 4 (Google Maps rendering cost/approach) remains explicitly PENDING** — tech-lead is researching this separately and will bring a recommendation back to the user via pm before it is written into requirements. No FR has been drafted for item 4; it is not approved and not scoped.
...
### Open Questions — Change Request 2026-07-12 (NOT YET RESOLVED)

Raised by pm while drafting FR-018/FR-019 for the toll-avoidance change request. These block final approval of FR-018 and FR-019 specifically (FR-020 and FR-021 are unaffected and ready for approval as drafted).

- **OQ-9 (No-toll-free-route fallback)** — Open. If "avoid tolls" is checked (FR-018) but no toll-free driving route exists at all between the given start/destination, what should the system do? Candidate options: fall back to a toll-inclusive route with a visible warning (mirroring FR-011's pattern), treat it as FR-012's "no viable option" case, or something else. Awaiting user decision.
- **OQ-10 (Toll re-entry question consequence)** — Open. FR-019 establishes that the system must ask the user whether a toll-road exit/re-entry pattern is acceptable, but not what happens based on the answer: does "not acceptable" exclude that candidate, trigger a targeted recalculation avoiding just that re-entry, merely get recorded/displayed without changing results, or something else? Awaiting user decision.
